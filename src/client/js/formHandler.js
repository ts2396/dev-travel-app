...
const getGeoNameInfo = async (place) => {
  ...
  const request = await fetch(
    `http://api.geonames.org/postalCodeSearchJSON?placename=${searchQuery}&username=${geoUsername}`
  );
  try {
...
const getWeatherDataUrl = async () => {
  try {
    const requestURLres = await fetch(
      'http://localhost:8060/weatherDetailsURL'
    );
...
const getWeatherData = async (reqUrl, details, time, place) => {
  ...
  if (time - currentDate > miliSecInweek) {
    requestUrl = `${reqUrl.apiUrl}&units=I&days=1&lat=${details.lat}&lon=${
      details.lng
      }&ts=${time / 1000}`;
  }
  try {
